...
const StepNineRef = ({formData, setFormData, step, setStep})=> {
    ...
    return (
        <div>
            <div className='w-11 md:w-11 lg:w-10 m-auto text-center'>
                <h1 className="text-900 text-4xl mb-6">Do you have a second mortgage?</h1>
                <div className="grid max-w-full m-auto w-full lg:w-26rem">
                    {
                        data.map((data, index)=> (
                            <div key={index} onClick={()=> {setState(data.name); setTimeout(()=> {setStep(step + 1)}, 1000); setFormData({...formData, isSecondMortage: data.name})}} className='col-6'>
